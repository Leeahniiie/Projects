...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            lst.append((x,y))
            variableDomainsDict[PAC] = lst
            variableDomainsDict[GHOST0] = lst
            variableDomainsDict[GHOST1] = lst
    obs = [i for i in range(MAX_NOISE + X_RANGE+Y_RANGE-1)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        evidenceDict:     An assignment dict {variable : value} for the
                          variables which are presented as evidence
                          (conditioned) in the inference query. 
        el-\iminationOrder: The order to eliminate the variables in.
        ...
        for variable in eliminationOrder:
            joined_fac = joinFactorsByVariable([item for element in list(joined_fac) for item in (element if isinstance(element, list) else [element])], variable)
        new_fac = joined_fac[-1]
        print(new_fac)

        for variable in eliminationOrder:
            if len(new_fac.unconditionedVariables()) == 1:
                break
            new_fac = eliminate(new_fac, variable)
        return(normalize(new_fac))
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        ghostInJail = ghostPosition == jailPosition
        if ghostInJail:
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        #print(self.beliefs)
        "*** YOUR CODE HERE ***"
        blankBelief = DiscreteDistribution()
        for pos in self.allPositions:
            blankBelief[pos] = 0
        ...
        for oldPos in self.allPositions:
            ...
            for newpos, prob in newPosDist.items():
                blankBelief[newpos] += self.beliefs[oldPos] * prob
        
        self.beliefs = blankBelief
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(gameState, oldPos) 
            new_parts.append(newPosDist.sample())
