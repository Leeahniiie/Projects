...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dict in livingGhostPositionDistributions:
            max_k = max(dict, key=lambda k: dict[k])
            dist = self.distancer.getDistance(pacmanPosition, max_k)
            if dist < closest_dist:
                closest_dist = dist
                most_likely = max_k
        closer_action = float(inf)
        pac_act = tuple()
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            pac_to_ghost = self.distancer.getDistance(successorPosition, most_likely)
            print(pac_to_ghost,closer_action)
            if pac_to_ghost < closer_action:
                closer_action = pac_to_ghost
                pac_act = action
        return pac_act
            
        #self.distancer.getDistance()
        #print(livingGhosts)
        #best_belief = self.observeUpdate()
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
