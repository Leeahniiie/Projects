...
def sentence3() -> Expr:
    ...
    Sent1 = PacmanAlive_1 % disjoin([PacmanAlive_0 & ~PacmanKilled_0, ~PacmanAlive_0 & PacmanBorn_0])
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    negationExp = ~conclusion & premise
    if findModel(negationExp):
        return False
    return True
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for comb in itertools.combinations(literals, 2):
        condition = ~comb[0] | ~comb[1]
        toBeConjoined.append(condition)
    return conjoin(toBeConjoined)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    Given:
        ...
        sensorModel(t, non_outer_wall_coords) -> Expr: function that generates
            the sensor model axioms. If None, it's not provided, so shouldn't be run.sensorModel
    ...
    for (x,y) in all_coords:
        wall_imp = PropSymbolExpr(wall_str, x, y)
        pac_imp = PropSymbolExpr(pacman_str, x, y, time=t) 
        implications = wall_imp >> ~pac_imp
        pacphysics_sentences.append(implications)
    PM_OneSquare = exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_outer_wall_coords]) 
    PM_OneAction =  exactlyOne([PropSymbolExpr(direction, time=t) for direction in DIRECTIONS])
    pacphysics_sentences.append(PM_OneSquare)
    pacphysics_sentences.append(PM_OneAction)
    if sensorModel:
        pacphysics_sentences.append(sensorModel(t,non_outer_wall_coords))
    if t > 0 and successorAxioms:
        pacphysics_sentences.append(successorAxioms(t, walls_grid, non_outer_wall_coords))
        #if PM_OneAction == None
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #successor axiom used for transition
    Time_0 = PropSymbolExpr(pacman_str, x0, y0, time=0)
    Time_1 = PropSymbolExpr(pacman_str, x1, y1, time=1)
    Action_0 = PropSymbolExpr(action0, time=0)
    Action_1 = PropSymbolExpr(action1, time=1)
    KB.append(pacphysicsAxioms(t=0, all_coords=all_coords, non_outer_wall_coords = non_outer_wall_coords, walls_grid= walls_grid, sensorModel = None, successorAxioms = allLegalSuccessorAxioms))
    ...
    KB.extend([Time_0, Action_0, Action_1])
    model1 = findModel(Time_1 & conjoin(KB))
    model2 = findModel(~Time_1 & conjoin(KB))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords])) # Pacman is still only in one place at a time

        goal = conjoin([~PropSymbolExpr(food_str, x, y, time=t) for x, y in food]) # If this is true, nowhere in any original food position is there food
        ...
        for x, y in food:
            foodNow = PropSymbolExpr(food_str, x, y, time=t)
            pacmanNow = PropSymbolExpr(pacman_str, x, y, time=t)
            FoodNext = PropSymbolExpr(food_str, x, y, time=t+1)
            KB.append(~FoodNext % ((foodNow & pacmanNow) | ~foodNow))
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.extend([pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms,allLegalSuccessorAxioms),fourBitPerceptRules(t=t, percepts=agent.getPercepts()),PropSymbolExpr(agent.actions[t], time=t),PropSymbolExpr(agent.actions[t], time=t)])
        possible_locations = []
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            entWall = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            entSpace = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y))
            if entWall:
                known_map[x][y] = 1
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in all_coords:
            pacmanThere = PropSymbolExpr(pacman_str, x, y, time=t)
            inconclusive = findModel(conjoin(KB + [pacmanThere]))
            wallThere = PropSymbolExpr(wall_str, x, y)
            if entails(conjoin(KB), wallThere):
                KB.append(wallThere)
                known_map[x][y] = 1
            if entails(conjoin(KB), pacmanThere):
                KB.append(pacmanThere)
                known_map[x][y] = 0
            if entails(conjoin(KB), ~wallThere):
                KB.append(~wallThere)
                known_map[x][y] = 0
